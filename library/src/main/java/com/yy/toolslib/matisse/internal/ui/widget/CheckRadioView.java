...
public class CheckRadioView extends AppCompatImageView {
    ...
    private void init() {
        ...
        mUnSelectUdColor = ResourcesCompat.getColor(
                getResources(), YyInflaterUtils.getIdByName(mContext, "color", "zhihu_check_original_radio_disable"),
                getContext().getTheme());
        setChecked(false);
    }
    ...
    public void setChecked(boolean enable) {
        if (enable) {
            setImageResource(YyInflaterUtils.getIdByName(mContext, "drawable", "ic_preview_radio_on"));
            mDrawable = getDrawable();
            mDrawable.setColorFilter(mSelectedColor, PorterDuff.Mode.SRC_IN);
        } else {
            setImageResource(YyInflaterUtils.getIdByName(mContext, "drawable", "ic_preview_radio_off"));
            mDrawable = getDrawable();
